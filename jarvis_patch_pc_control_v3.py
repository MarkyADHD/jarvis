...
def backup(path):
    ...
    backup_path = path.with_name(f"{path.stem}_backup_before_pc_control_v3_{stamp}{path.suffix}")
...
def remove_old_v3_precheck(text):
    pattern = (
...
def add_precheck_after_name(text):
    text = remove_old_v3_precheck(text)
...
def patch_log_line(text):
    if "PC Control V3 loaded" in text:
        return text

    replacements = [
        ("Operator V1 + Operator V2.1 loaded", "Operator V1 + Operator V2.1 + PC Control V3 loaded"),
        ("Operator V1 + Operator V2 loaded", "Operator V1 + Operator V2 + PC Control V3 loaded"),
        ("Operator V1 loaded", "Operator V1 + PC Control V3 loaded"),
        ("Desktop V2 loaded", "Desktop V2 + PC Control V3 loaded"),
    ]
...
def main():
    ...
    print("Jarvis PC Control V3 patch applied.")
