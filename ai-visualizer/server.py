...
_CHAT_TOKEN_FILE = Path(r"C:\AI-Agent\.remote_chat_token")
_chat_token_cache = None


def _chat_token():
    """The same token jarvis_remote_chat.py guards /ask and /voice with.
    Handed to the faces so the in-HUD chat bar (core.js) can call that
    server directly -- read once and cached, same trust boundary as the
    rest of /config (only reachable on localhost or this machine's own
    Tailscale devices)."""
    global _chat_token_cache
    if _chat_token_cache is None:
        try:
            _chat_token_cache = _CHAT_TOKEN_FILE.read_text(encoding="utf-8").strip()
        except OSError:
            _chat_token_cache = ""
    return _chat_token_cache
...
if __name__ == "__main__":
    ...
    try:
        # 0.0.0.0, not 127.0.0.1: lets the face HUD be reached over Tailscale
        # from the phone too. Read-only (GET /state, /config, static files
        # only, no control endpoints), so this is safe to expose to the
        # private Tailscale mesh even without its own auth token.
